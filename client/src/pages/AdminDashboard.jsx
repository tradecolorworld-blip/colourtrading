...
const AdminDashboard = () => {
    ...
    // 🟢 IST Formatter (Forces India Timezone & AM/PM)
    ...
    return (
        <div className="min-h-screen bg-[#0b0e14] text-[#e2e8f0] p-4 md:p-8 font-sans">
            ...
            {/* --- PERFORMANCE GRID (TODAY / YESTERDAY / WEEK) --- */}
            ...
            <div className="bg-[#151921] rounded-[32px] border border-white/5 overflow-hidden shadow-2xl">
                <div className="p-8 border-b border-white/5 bg-white/5 flex justify-between items-center">
                    ...
                    <div className="text-[10px] bg-green-500/20 text-green-400 px-4 py-1.5 rounded-full font-black uppercase tracking-widest">
                        Latest Purchase First
                    </div>
                </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {stats.vipList.map((vip, index) => {
                                ...
                                const datePart = fullDateIST.split(',')[0];
                                const timePart = fullDateIST.split(',')[1];

                                return (
                                    <tr key={index} className="hover:bg-white/[0.03] transition-colors group">
                                        <td className="p-8">
                                            <p className="text-sm font-black text-orange-400">{datePart}</p>
                                            <p className="text-[11px] font-bold text-gray-500 uppercase">{timePart}</p>
                                        </td>
...
// --- SUB-COMPONENT: PERFORMANCE CARD ---
const StatCard = ({ title, data, color }) => (
    <div className={`bg-[#151921] p-6 rounded-[28px] border-b-4 ${color} shadow-2xl hover:translate-y-[-5px] transition-transform duration-300`}>
        <div className="flex justify-between items-start mb-6">
            <h4 className="text-gray-400 font-black text-[11px] uppercase tracking-[0.2em]">{title} Overview</h4>
            <div className={`w-2 h-2 rounded-full animate-pulse ${color.replace('border', 'bg')}`}></div>
        </div>
        <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded-2xl">
                <p className="text-[10px] font-bold text-gray-500 uppercase mb-1">New Users</p>
